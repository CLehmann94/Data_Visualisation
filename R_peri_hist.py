...
def helpmessage():
    print('')
    print('A histogram of perihelion radii of stars within ')
...
# looking for help?
    ...
    output = 'Rperi_hist.pdf'
    pdf = matplotlib.backends.backend_pdf.PdfPages(output)

    fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1, figsize=(6, 9))

    histogram_plot(R_peri[metal_rich], ax1, xlim=[4.01, 8.99], x_notags=True,
                   legend_handle="[Fe/H]>0.3", normalise=True)
    histogram_plot(R_peri[solar_metal], ax1, xlim=[4.01, 8.99], x_notags=True,
                   legend_handle="-0.3<[Fe/H]<0.3", normalise=True)
    histogram_plot(R_peri[metal_poor], ax1, xlim=[4.01, 8.99],
                   ylim=[-0.01, 0.1999], x_notags=True,
                   ylabel="Norm. distribution", legend_handle="[Fe/H]<-0.3",
                   normalise=True, legend_loc="upper right")

    histogram_plot(R_peri[young], ax2, xlim=[4.01, 8.99],
                   legend_handle="age<4Gyrs", normalise=True)
    histogram_plot(R_peri[medium], ax2, xlim=[4.01, 8.99],
                   legend_handle="4Gyrs<age<8Gyrs", normalise=True)
    histogram_plot(R_peri[old], ax2, xlim=[4.01, 8.99], ylim=[-0.01, 0.1999],
                   ylabel="Norm. distribution",
                   xlabel=r"$r_\textrm{peri}$ [kpc]",
                   legend_handle="age>8Gyrs", normalise=True,
                   legend_loc="upper right")
    ax1.axvline(6, color='black', ls="--")
    ax1.axvline(8, color='black', ls="--")
    ax2.axvline(6, color='black', ls="--")
    ax2.axvline(8, color='black', ls="--")
